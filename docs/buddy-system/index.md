...
链表提供只读（`iter`）与可变（`iter_mut`）两种迭代器。借助可变迭代器，用户可以在遍历过程中定位、访问并移除任意节点，使用方式与普通链表一致，无需额外内存分配，兼具灵活性与高效性。
...
1. 计算当前地址和剩余长度能放入的最大阶次块（同时满足地址对齐和剩余空间限制）。
2. 将该块插入对应阶次的空闲链表。
...
3. 从该阶开始逐级向上合并伙伴块：
   ...
   - 在同阶的空闲链表中查找此伙伴地址：
     - 若找到，则将伙伴块从链表中移除，与当前块合并为更大阶次的块，新的块起始地址为两者中较小者，继续在下一阶查找；
     - 若未找到，则结束合并。
