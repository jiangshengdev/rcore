...
原子指令扩展，命名为「A」，包含了能够原子性地执行读改写内存操作的指令，以支持在同一内存空间中运行的多个 RISC-V 硬件线程（harts）之间的同步。提供的两种形式的原子指令是保留载入/条件存储指令和原子取值-操作内存指令。两种类型的原子指令都支持各种内存一致性序，包括无序、获取语义（acquire）、释放语义（release）和顺序一致性语义。这些指令允许 RISC-V 支持 RCsc 内存一致性模型。（Gharachorloo et al., 1990）
