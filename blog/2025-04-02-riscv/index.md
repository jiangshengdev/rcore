...
**等待中断指令（WFI）** 通知硬件实现当前硬件线程（hart）可以暂停运行，直到可能需要处理中断。执行 WFI 还可用于建议硬件平台优先将合适的中断分配到该
...
- 如果 **rs1** ≠ `x0` 且 **rs2** ≠ `x0`，则该屏障只对由整数寄存器 **rs2** 指定的地址空间中、与 **rs1**
  对应虚拟地址相关的页表叶子条目的读写操作进行排序（不包含全局映射 global 条目的访问），并使该地址空间中与该虚拟地址相关的非全局翻译缓存条目失效。
